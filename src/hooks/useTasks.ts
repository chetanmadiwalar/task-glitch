...
export function useTasks(): UseTasksState {
  ...
  const derivedSorted = useMemo<DerivedTask[]>(() => {
    ...
    const sorted = sortDerived(withRoi);
  ...
  const metrics = useMemo<Metrics>(() => {
    ...
    if (doneTasks.length === 0) return INITIAL_METRICS;

    const totalRevenue = computeTotalRevenue(doneTasks);
    const totalTimeTaken = doneTasks.reduce((s, t) => s + t.timeTaken, 0);
    const timeEfficiencyPct = computeTimeEfficiency(tasks); // Done / All
    const revenuePerHour = computeRevenuePerHour(doneTasks);
    const averageROI = computeAverageROI(doneTasks);
